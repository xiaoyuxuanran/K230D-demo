...
fpioa = None
HT_CTRL = None
_fpioa_done = False   # FPIOA 只能初始化一次，重复创建会重置所有引脚功能
_shared_tp = None      # TOUCH(0) 全局单例，避免重复创建导致 I2C 冲突
...
def init_player_hardware():
    ...
    # FPIOA 只在首次调用时初始化，重复创建会重置 DSI/I2C/音频引脚
    if not _fpioa_done:
        fpioa = FPIOA()
        fpioa.set_function(10, FPIOA.GPIO10)
        _fpioa_done = True

    # 功放引脚只创建一次，后续复用（避免同一引脚多实例冲突）
    if HT_CTRL is None:
    ...
    # TOUCH(0) 全局单例，只创建一次，避免多个 I2C 主控冲突
...
CONTENT_X   = 232
CONTENT_W   = 542
CONTENT_CX  = CONTENT_X + CONTENT_W // 2  # ~503
...
EQ_COUNT   = 20
EQ_MAX_H   = 30
EQ_START_X = CONTENT_X + 30

# 音量（竖向滑杆，右侧。下移避免与主题按钮重叠）
VOL_X      = 730
VOL_Y      = 185
VOL_W      = 8
VOL_H      = 110
...
BTN_PREV_X = CONTENT_CX - 115
BTN_NEXT_X = CONTENT_CX + 115
...
def draw_theme_button():
    label = "浅色" if is_dark_theme else "深色"
    bg = (242, 247, 252) if is_dark_theme else (26, 38, 55)
    fg = (26, 38, 55) if is_dark_theme else (246, 250, 255)
    knob_x = THEME_X + 74 if is_dark_theme else THEME_X + 20
    draw_pill(THEME_X, THEME_Y, THEME_W, THEME_H, bg)
    img.draw_circle(knob_x, THEME_Y + THEME_H // 2, 13, color=C_ACCENT2, thickness=1, fill=True)
    img.draw_string_advanced(THEME_X + 16, THEME_Y + 10, 14, label, color=fg)


def draw_exit_button():
    draw_pill(EXIT_X, EXIT_Y, EXIT_W, EXIT_H, C_ACCENT)
    img.draw_string_advanced(EXIT_X + 25, EXIT_Y + 9, 16, "退出", color=C_WHITE)
...
def draw_song_list_panel():
    ...
    if music_count == 0:
        ...
        img.draw_string_advanced(LIST_X + 22, LIST_Y + 118, 13, "请放入", color=C_SUBTEXT)
        img.draw_string_advanced(LIST_X + 22, LIST_Y + 138, 13, "/data/code/music/", color=C_SUBTEXT)
        return
...
def draw_top_info():
    """顶部歌曲信息"""
    if music_count == 0:
        draw_text_centered(CONTENT_X, TITLE_Y + 20, CONTENT_W, "请放入 WAV 文件到 /data/code/music/", 16, C_SUBTEXT)
        return
...
def draw_rotating_vinyl(cx, cy, r, angle):
    ...
    # 纹路（适配小半径唱片）
    for gr in [r-5, r-10, r-16, r-22, r-28, r-34, r-40, r-46, r-52]:
        img.draw_circle(cx, cy, gr, color=C_GROOVE, thickness=1, fill=False)

    # 光泽
    img.draw_circle(cx - 17, cy - 20, r // 3, color=(33, 43, 58), thickness=1, fill=False)
    for i in range(4):
        sy = cy - r + 15 + i * 5
        sw = int((r - 28) * (0.22 + i * 0.13))
        if sw > 2:
    ...
    # 标签文字
    img.draw_string_advanced(cx - 16, cy - 8, 12, "MUSIC", color=(255, 250, 245))
    img.draw_string_advanced(cx - 12, cy + 6, 9, "33RPM", color=(255, 230, 205))
...
def draw_equalizer(x, y, w, count, gap, max_h):
    """均衡器柱状图"""
    img.draw_string_advanced(x, y - 20, 12, "SPECTRUM", color=C_MUTED)
...
def handle_touch(points):
    ...
    if is_touch_down and is_inside_rect(x, y, EXIT_X - 6, EXIT_Y - 6, EXIT_W + 12, EXIT_H + 12):
        player_should_exit = True
        debounce_until = time.ticks_add(now, BTN_DEBOUNCE_MS)
        return

    # ---- 主题按钮：仅按下瞬间触发 ----
    if is_touch_down and is_inside_rect(x, y, THEME_X - 6, THEME_Y - 6, THEME_W + 12, THEME_H + 12):
